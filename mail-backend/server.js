...
app.get("/api/seed", async (req, res) => {
  try {
    ...
    if (studentCount > 0 || staffCount > 0) {
      return res.json({ message: "Database already seeded!", students: studentCount, staff: staffCount });
    }
    ...
    res.json({ success: true, message: "Database seeded!", students: students.length, staff: staffMembers.length });
  } catch (error) {
